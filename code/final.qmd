...
sst_clean <- sst |> 
  mutate(date = ymd(date)) |>                   # Convert to Date class
  mutate(
    year = year(date),                          # Extract year
    month = month(date, label = TRUE, abbr = TRUE)  # Extract month (abbreviated)
  ) |> 
  filter(year >= 2018 & year <= 2023) |>        # Keep only 2018–2023
  group_by(year, month) |> 
